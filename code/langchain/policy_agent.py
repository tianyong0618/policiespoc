...
"""
    ...
    def build_analysis_prompt(self, intent_info, matched_user, all_policies, all_jobs, all_courses):
        ...
        prompt += "   - 补贴申请情况：如可按POLICY_A02申请1500元技能补贴\n"
        prompt += "   - 工作模式：如兼职模式满足灵活时间需求\n"
        prompt += "   - 收入情况：如课时费+补贴双重收入\n"
        ...
        prompt += "   - 补贴申请情况\n"
        prompt += "   - 学习难度与基础匹配度\n"
        prompt += "5. 为每个推荐的政策、岗位和课程提供优先级（1-5，5最高）\n"
        prompt += "6. 生成主动建议，包括下一步操作\n\n"
        prompt += "示例推荐理由：\n"
        prompt += "推荐理由：①持有中级电工证符合岗位要求，可按POLICY_A02申请1500元技能补贴（若以企业在职职工身份参保）；②兼职模式满足灵活时间需求，课时费+补贴双重收入；③岗位特点'传授实操技能'，与您的经验高度匹配。\n\n"
        prompt += "输出格式：\n"
        prompt += "{\n"
