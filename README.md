...
2. **消息发送**：支持回车键和按钮发送
3. **历史管理**：管理对话历史记录
4. **结果展示**：结构化显示AI回复，使用卡片式布局
5. **性能监控**：显示LLM调用时间和响应时间
6. **会话管理**：支持多会话切换和删除
7. **卡片式布局**：场景化卡片设计，包含名称和ID的统一显示格式

#### 卡片显示格式
- **场景一**：创业扶持政策卡片，包含符合/不符合条件的政策，主动建议显示为"推荐联系《创业孵化基地管理员》（JOB_A01）"
- **场景二**：岗位推荐卡片，显示"职业技能培训讲师（JOB_A02）"格式，包含"优先推荐"标签
- **场景三**：多重政策卡片，显示"退役军人创业税收优惠（POLICY_A06）"格式
- **场景四**：课程推荐卡片，显示"电商运营入门实战班（COURSE_A01）"格式，包含学历要求、基础要求、课程内容和推荐理由
